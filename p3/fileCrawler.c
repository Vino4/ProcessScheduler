...
static int applyRe(char *dir, RegExp *reg, TreeSet *ts) {
    ...
    int status = 1;
    newDir = strdup(dir); 
    //printf("applyRe received %s\n", dir);
    //printf("Obtained file name is: %s\n", basename(newDir));
...
void * workerFunction(UNUSED void * blah){
    ...
    */
    while (((!mainThreadDone) || (zThreads < CRAWLER_THREADS))) {
        ...
        while(!tsll_size(ll)){
            zThreads++;
            //printf("incremeanted zThreads on size %ld\n", tsll_size(ll));
            ...
            zThreads--;
            //printf("decremeanted zThreads on size %ld\n", tsll_size(ll));    
        }
        //printf("out of condWaitLoop on size %ld\n", tsll_size(ll));
