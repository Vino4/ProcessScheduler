/*
*
*   procNode - Functions to handle the proccess structures
...
pNode *pNode_create(char buf[], int argc){
	...
	if (newNode == NULL){
		p1perror(2, "pNode_create: failed to malloc new node");
		return newNode;
	}

	newNode->argvData = (char**) calloc(argc +1, sizeof(char*));
	if (newNode->argvData == NULL){
		p1perror(2, "pNode_create: failed to malloc argvData");
		return NULL;
	}

	int i, idx = 0;
	for(i = 0; i < argc; i++){
		idx = p1getword(buf, idx, argBuffer);
		newNode->argvData[i] = p1strdup(argBuffer); 
...
/*
*	pNode_updatePriority - calculates a priority for the node using its log of total IO reads and writes 
*		less the log of its total CPU time less the number of runs (for aging)
*/

void pNode_updatePriority(pNode * node){
	double logIO = 0;
	double logCPU = 0;
	int totalIO = node->stat_reads + node->stat_writes;
	int totalCPU = node->stat_cpuKrnlTime + node->stat_cpuUsrTime;
	...
	node->priority = (logIO - logCPU) - (node->number_of_runs) * 0.1;
}

/*
*	pNode_getPriority - returns the priority of a proc 
*		returns 10000 otherwise.
*/

double pNode_getPriority(pNode *node){
	if (node != NULL)
...
void pNode_run(pNode *node){
	...
	if (pid == -1){
		...
		if (errX < 0)
			p1perror(2, "pNode_run: failed to execvp");
